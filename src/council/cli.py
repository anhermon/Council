...
def review(
    ...
    async def _review():
        ...
        # Scale with file count but cap at max_concurrent_reviews
        concurrency_limit = min(
            settings.max_concurrent_reviews, max(2, len(files_to_review) // 3)
        )
...
def housekeeping():
    ...
    for pattern, addition in patterns_to_check.items():
    ...
    for item in project_root.iterdir():
        if item.is_file():
            if item.name not in standard_files and item.suffix in (".tmp", ".temp", ".log", ".bak"):
                ...
                if stripped.startswith("#") and len(stripped) > 1:
                    ...
                    if i + 2 < len(lines):
                        ...
                        if next_two_comments:
                            ...
                            # Note: We'll report but not auto-delete for safety

        except Exception as e:
            click.echo(f"    ⚠️  Error analyzing {file_path.relative_to(project_root)}: {e}", err=True)

    if commented_code_blocks > 0:
        click.echo(
            f"  ℹ️  Found {commented_code_blocks} potential commented code blocks (manual review recommended)",
            err=True,
        )
    else:
        click.echo("  ✅ No obvious commented code blocks found", err=True)
    ...
    # Phase 3.1: Docstring Audit (simplified check)
    ...
    if functions_without_docs > 0:
        click.echo(
            f"  ℹ️  Found {functions_without_docs} public functions without docstrings (manual review recommended)",
            err=True,
        )
    else:
    ...
    if readme_path.exists():
        ...
        if "uv run council" in readme_content or "council review" in readme_content:
            click.echo("  ✅ README mentions correct CLI commands", err=True)
        else:
            click.echo("  ⚠️  README may need updating for CLI commands", err=True)

        # Check if setup instructions mention uv sync
        if "uv sync" in readme_content:
            click.echo("  ✅ README mentions uv sync", err=True)
        else:
            click.echo("  ⚠️  README may need to mention uv sync", err=True)
    else:
        click.echo("  ⚠️  README.md not found", err=True)
    ...
    # Read pyproject.toml to get project info
